...
def lancement():
    ...
    for i in range(2):
        rect = titres[i].get_rect(center=((2 * (i + 1)) * (screen_width / 6), 3 * (screen_height / 12)))
        titres_principaux.append(rect)
        if i == 0:
            titresr.extend([titres[j].get_rect(center=(titres_principaux[i].centerx - (screen_width/12.8), titres_principaux[i].centery + (j - 2) * (screen_height/10.8))) for j in range(3, 7)])
        else:
            titresr.extend([titres[j].get_rect(center=(titres_principaux[i].centerx - (screen_width/12.8), titres_principaux[i].centery + (j - 4) * (screen_height/10.8))) for j in range(5, 7)])
    ...
    select_rect = select.get_rect(center=(titres_principaux[0].centerx - (screen_width/64), titres_principaux[0].centery))
    ...
    rect_quitter = titres[9].get_rect(center=(screen_width / 2, screen_height - (screen_height/5.4)))
    rect_options = titres[7].get_rect(center=(screen_width / 2, screen_height / 10))
    ...
    while cdt:
        ...
        screen.blit(text_rem, text_rem.get_rect(center = (rect_quitter.centerx-(screen_width/3.5),screen_height-(screen_height/10.8))).inflate(-200,-20))
        ...
        for j in range(len(titresr)):
            ...
            text_rect = text.get_rect(center=(titresr[j].right + (screen_width/24), titresr[j].centery))
