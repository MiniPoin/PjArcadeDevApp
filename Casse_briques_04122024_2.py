...
def creation(screen_width, screen_height):
            ...
            for j in range(6):
                ...
                if a == 1 and nb < nbm:  # nombre minimuim de briques
                    ...
                    if pos2 not in positions and nb < 100:
                        nb += 1
                        val2 = Rectangle(pygame.Rect(pos2[0], pos2[1], (screen_width/12), (screen_height/36)), 1)
                        vie_proba = random.randint(1, 100)
                        if vie_proba % 2 == 0:
                            val2.set_pv(2)
                        elif vie_proba % 3 == 0:
    ...
    return l
def bouge(r,dt,screen,screen_width,touchesx):
    ...
    if keys[touchesx[0]] and r.cote_pos()[2] > 0:
        r.deplacement(-700* dt, 0)
    if keys[touchesx[1]] and r.cote_pos()[3] < screen_width:
        r.deplacement(700* dt, 0)
    return
def bouge_avant_partie(r,c,keys,screen,screen_width, screen_height,dt,touchesx):
    ...
    if keys[touchesx[1]] and r.cote_pos()[3] < screen_width:
        r.deplacement(500* dt, 0); c.deplacement(500* dt, 0)
    return
def afficher_score(score, start_g,font,screen,screen_width, screen_height,vie):
    # Créer le texte en fonction du score
    texte = font.render(f"Score: {score}", True, (255, 255, 255))  # Texte en blanc
    texte2 = font.render(f"Vie: {vie}", True, (255, 255, 255))  # Texte en blanc
    # Positionner le texte sur l'écran
    screen.blit(texte, (screen_width/50, screen_height - (screen_height/10)))
    screen.blit(texte2, (screen_width/50, screen_height - (screen_height/6)))
...
def fin_jeu(font,screen,l, screen_width, screen_height):
    ...
    if len(l) == 0:
        screen.blit(texte2, (screen_width/2 - texte2.get_width()//2, screen_height/2))
    else:
        screen.blit(texte, (screen_width/2 - texte.get_width()//2, screen_height/2))
    pygame.display.flip()
...
def lancement(touchesx):
    ...
    font = pygame.font.Font(None, screen_width//26)
    ...
    new_width = int(cercleR.get_width() * 0.09)
    new_height = int(cercleR.get_height() * 0.09)
        ...
        if running == True:
            if not start_g:
                ...
                if keys[pygame.K_SPACE]:
                    start_g = True
                bouge_avant_partie(plateforme, cercle, keys,screen,screen_width, screen_height,dt,touchesx)
            ...
            afficher_score(score, start_g,font,screen, screen_width, screen_height,vie)
            if start_g:
                pygame.draw.rect(screen, (0, 0, 0), plateforme.get_rect())
                bouge(plateforme,dt,screen, screen_width,touchesx)
